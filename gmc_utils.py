...
def compute_cache_score(
    ...
    score = cfg.score_s1_weight * s1 + cfg.score_s3_weight * s3
    ...
    if (
        ...
        score = score + cfg.score_drift_weight * _normalize_per_batch(drift)
    ...
    if (
        ...
        score = score + cfg.score_anchor_weight * _normalize_per_batch(anchor)
...
def gather_tokens(tokens: torch.Tensor, indices: torch.Tensor) -> torch.Tensor:
    idx = indices.unsqueeze(-1).expand(-1, -1, tokens.shape[-1])
